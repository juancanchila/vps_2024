...
export class ResumenDatosEnvioComprasPage implements OnInit {
  ...
  async slideNext() {
    ...
    if (
      this.AuxCarrosDisponibles['length'] == 0 &&
      this.AuxMotosDisponibles['length'] == 0
    ) {
      ...
      for (var i = 0; i < this.direccionDestino.length; i++) {
        if (
          this.FormSend.value['field_barrio_destino'] ==
          this.direccionDestino[i].name
        ) {
          barrioExiste = true;
          let n = i;
      ...
      if (barrioExiste == false) {
        const alert = await this.alertController.create({
          ...
          message:
            'Barrio(s) no identificado, Debes seleccionar tu barrio de la lista',
          // al hacer check, vamos a establecer una variable y al darle aceptar preguntamos si esa varibale esta definida si esta se continua
          buttons: [
            {
              text: 'aceptar',
  ...
  async irAPagar() {
    ...
    if (this.aux == 'false') {
      ...
      if (
        this.FormSend.invalid ||
        this.FormSend.value['field_metodo_de_pago'] == ''
      ) {
        const alert = await this.alertController.create({
        ...
        for (let i = 0; i < this.ciudades.length; i++) {
          if (this.ciudades[i]['name'] == localStorage.getItem('locacion')) {
            ...
            console.log(
              this.AuxMotosDisponibles['length'],
              'lengt de vector motos'
            );
            if (this.AuxMotosDisponibles.length == 0) {
              if (this.AuxCarrosDisponibles.length == 0) {
                const alert = await this.alertController.create({
                  ...
                  message:
                    'En este momento no tenemos auxiliar disponible, no podemos crear tu orden',
                  // al hacer check, vamos a establecer una variable y al darle aceptar preguntamos si esa varibale esta definida si esta se continua
                  buttons: [
                    {
                      text: 'aceptar',
  ...
  async ngOnInit() {
    ...
    try {
      ...
      this.FormSend.controls.field_observaciones.setValue(
        localStorage.getItem('enviarProductos')
      );

      //this.FormSend.controls.field_contacto.setValue(this.auth.resumen.field_direccion_entrega['0']['value']);

      this.FormSend.controls.field_medio_de_transporte.setValue(
  ...
  async WillEnter() {
    try {
      ...
      this.auth.getListBarriosSeleccion().subscribe(
        async (data) => {
          ...
          if (this.direccionDestino.length === 1) {
            this.FormSend.controls.field_barrio_destino.setValue(
              this.direccionDestino[0]['name']
            );
            localStorage.setItem(
              'tarifaDestino',
              this.direccionDestino[0]['field_tarifa']
            );
            localStorage.setItem(
              'tarifaExternaDestino',
              this.direccionDestino[0]['field_tarifa_externa']
            );
            localStorage.setItem(
              'imgBarrioDestino',
              this.direccionDestino[0]['field_imagen_barrio']
            );

            // Agregando la zona de destino
            console.log(
              'selected----> destino',
              this.direccionDestino[0]['field_zona_a']
            );
          ...
          if (
            this.direccionDestino.length === 1 &&
            this.direccionDestino[0]['name'] === 'San Andrés'
          ) {
            this.FormSend.controls.field_barrio_destino.setValue(
              this.direccionDestino[0]['name']
            );
