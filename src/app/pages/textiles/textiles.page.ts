...
@Component({
  ...
  templateUrl: './textiles.page.html',
  styleUrls: ['./textiles.page.scss'],
})
export class TextilesPage implements OnInit {
...
await alert.present();
  ...
  ngOnInit() {
    ...
    this.auth.getMessageTextiles().subscribe(async data => {
      this.message_textiles = data[0]['body'];
      console.log(data, 'Data received in component');

      // Crear y mostrar la alerta después de recibir los datos
      const alert = await this.alertController.create({
        header: 'Importante',
        message: this.message_textiles || 'No se recibió el mensaje de la API', // Muestra un mensaje por defecto si no hay datos
        buttons: [
          {
            text: 'Aceptar',
            handler: () => {
    ...
    this.auth.seleccionarSliderTextiles().subscribe(res =>{
      console.log(res, ' aqui slider');
     this.slider=res[0]['field_imagen_textiles'];
  ...
  async WillEnterOrigen() {
    try {
      ...
      if (this.direccion.length === 1 && this.direccion[0]['name'] !== 'San Andrés') {
        ...
        localStorage.setItem('zona_origen', this.direccion[0]['field_zona_a']); // Cambiado a this.direccion[0]
        ...
        console.log('selected----> origen', this.direccion[0]['field_zona_a']); // Cambiado a this.direccion[0]
  ...
  async WillEnter() {
    try {
      ...
      // Manejar las condiciones basadas en la longitud de direccionDestino
      if (this.direccionDestino.length === 1) {
        ...
        // Si no hay barrios
        console.log('No hay barrios disponibles');
        this.FormSend.controls.field_barrio_destino.setValue('');
        this.ocultarInput = false;
        this.bloquearInputBarrioDestino = false;
        document.getElementById('itemOrigen').style.visibility = "visible"; // Mostrar
